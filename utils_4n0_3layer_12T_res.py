...
def construct_model(config):
    ...
    # 使用DSTAGNN的STAG图
    adj = np.load("./data/XiamenBRT/brt2023_Astag.npz")['data']
    ...
    net = stsgcn(
        ...
        prefix="", rho=1, predict_length=my_length
    )
...
def generate_from_train_val_test(data, transformer):
    ...
    for key in ('train', 'val', 'test'):
        ...
        x, y = generate_seq(data[key], my_length, my_length)
        if transformer:
            x = transformer(x)
            y = transformer(y)
        if mean is None:
...
def generate_from_data(data, length, transformer):
    ...
    for line1, line2 in ((0, train_line),
        ...
        x, y = generate_seq(data['data'][line1: line2], my_length, my_length)
        if transformer:
            x = transformer(x)
            y = transformer(y)
        if mean is None:
