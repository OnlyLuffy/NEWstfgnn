...
def sthgcn_layer_individual(data, adj,
    ...
    # data_left_c1 = mx.sym.Convolution(
    #                             data=data_temp, 
    #                             num_filter=num_of_features, 
    #                             kernel=(1, 2), stride=(1, 1), dilate=(1,1))
    # data_left = mx.sym.sigmoid(mx.sym.Convolution(
    #                             data=data_left_c1, 
    #                             num_filter=num_of_features, 
    #                             kernel=(1, 3), stride=(1, 1), dilate=(1,1)))
    # data_right_c1 = mx.sym.Convolution(
    #                             data=data_temp, 
    #                             num_filter=num_of_features, 
    #                             kernel=(1, 2), stride=(1, 1), dilate=(1,1))
    # data_right = mx.sym.Convolution(
    #                             data=data_right_c1, 
    #                             num_filter=num_of_features, 
    #                             kernel=(1, 3), stride=(1, 1), dilate=(1,1))
    data_left = mx.sym.sigmoid(mx.sym.Convolution(
                                data=data_temp, 
                                num_filter=num_of_features, 
                                kernel=(1, 2), stride=(1, 1), dilate=(1,3)))
    data_right = mx.sym.Convolution(
                                data=data_temp, 
                                num_filter=num_of_features, 
                                kernel=(1, 2), stride=(1, 1), dilate=(1,3))
    data_time_axis = data_left * data_right
    # data_new_x = data_temp[:,:,:,:21] * (1 - data_left)
    # data_time_axis = data_time_axis + data_new_x
    # shape is (B, T-3, N, C)
    ...
    for i in range(T - 3):
        ...
        # shape is (B, N, C')
        t = mx.sym.swapaxes(t, 0, 1) # 交换t的第0轴和第1轴 (N, B, C') -> (B, N, C')
...
def stsgcn(data, adj, label,
    ...
    # (B, 1, N)
    need_concat = []
    for i in range(predict_length): # 超参数设置的预测长度
        need_concat.append(
            output_layer(
